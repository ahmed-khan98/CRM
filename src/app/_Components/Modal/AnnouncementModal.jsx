...
const AnnouncementModal = ({ isOpen, closeModal, data, refetch }) => {
                ...
                <div className="flex items-center gap-1 md:gap-4">
                  ...
                  <div>
                    <h2 className="text-lg mdtext-2xl font-bold">
                      {data ? "Edit Announcement" : "Add New Announcement"}
                    </h2>
                  </div>
                </div>
                <motion.button
  ...
  {/* Message Field (Converted to Textarea) */}
  <div>
    <label className="block text-sm font-medium text-gray-700 mb-1">
      Message
    </label>
    <Field
      as="textarea"  // 👈 'as' use karna hai textarea ke liye
      rows="4"       // 👈 Default height set karne ke liye
      name="message"
      className={`w-full px-4 py-2 ${
        errors.message && touched.message
          ? "border border-zinc-500 focus:border-zinc-500"
          : "border border-gray-200 focus:border-blue-500"
      } rounded-xl focus:outline-none transition-colors resize-none`} // 'resize-none' se user box ko khinch nahi sakega (optional)
    />
    <ErrorMessage
      name="message"
      component="div"
