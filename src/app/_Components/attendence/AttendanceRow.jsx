...
const AttendanceRow = ({ row }) => {
  ...
  const effectiveTodayStr = currentHour < 5 
    ? now.clone().subtract(1, "days").format("YYYY-MM-DD") 
    : now.format("YYYY-MM-DD");
  ...
  const isDiscrepancy = hasTimeIn && !hasTimeOut && (hoursSinceIn > 20 || isPastShift);
  ...
  const renderTimeOut = () => {
    ...
    return <span className="text-gray-300 text-[11px] font-bold">--:--</span>;
  };
