...
export default function AuctionCard() {
  ...
  const { data, error, isLoading, refetch, isSuccess, isFetching } = useGetTodayyAuctionsProductQuery()
  ...
  const handleRefetch = () => {
    try {
      if (isSuccess && !isFetching && refetch) {
        console.log("Refetching auction data...")
        refetch()
      } else {
        console.warn("Cannot refetch: Query not ready or already fetching")
      }
    } catch (error) {
    ...
    <div className="bg-[#FFFFFF] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4  gap-6 container mx-auto p-4 pt-44 md:pt-33 ">
      {isLoading ? (
        ...
        filteredProducts?.filter((item, index) => !item?.isSold).map((item, index) => (
          <ProductCard key={item?._id ?? `auction-${index}`} item={item} index={index} handleRefetch={handleRefetch} />
        ))
